...
% Standalone Overleaf configuration
\usepackage{graphicx}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage[a4paper,top=1in,bottom=1in,left=1.15in,right=1in]{geometry}
\usepackage{xcolor}
\usepackage{fancyhdr}
\usepackage{hyperref}
\usepackage{setspace}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small TiTalks}
\fancyhead[R]{\small UCS503 - Software Engineering}
\fancyfoot[C]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

% Commands used by the supplied TIET report template
\makeatletter
...
\newcommand{\TitlePageHeader}[1]{\gdef\@titlepageheader{#1}}
\newcommand{\ProjectTitle}[1]{\gdef\@projecttitle{#1}}
\newcommand{\ProjectSubTitle}[1]{\gdef\@projectsubtitle{#1}}
\newcommand{\TitlePageSubText}[1]{\gdef\@titlepagesubtext{#1}}
\newcommand{\AdvisorName}[1]{\gdef\@advisorname{#1}}
\newcommand{\TitlePageFooterText}[1]{\gdef\@titlepagefootertext{#1}}

\renewcommand{\maketitle}{%
  \begin{titlepage}
    \centering
    {\Large\bfseries \@titlepageheader\par}
    \vspace{1.4cm}
    {\color{tietred}\rule{0.85\textwidth}{1.2pt}\par}
    \vspace{1.2cm}
    ...
    {\large \@author\par}
    \vspace{0.8cm}
    {\large \@titlepagesubtext\par}
    \vfill
    {\large\bfseries Faculty/Teacher\par}
    \vspace{0.25cm}
    {\large \@advisorname\par}
    \vfill
    {\color{tietred}\rule{0.85\textwidth}{1.2pt}\par}
    \vspace{0.6cm}
    {\@titlepagefootertext\par}
  \end{titlepage}
  \clearpage
}
\makeatother
...
\begin{tikzpicture}[
  ...
  current/.style={-{Latex[length=2.5mm]}, thick, draw=tietred},
  planned/.style={-{Latex[length=2.5mm]}, thick, dashed, draw=gray!75}
]
  \node[box] (user) {TIET Student / Club User};
  \node[box, below=of user] (ui) {React Interface and React Router\\Feed, Profiles, Reels, Messages, Events and Settings};
  \node[box, below=of ui] (state) {Client-Side Application State\\Likes, Comments, Follows, Posts and Interface Feedback};
  \node[box, below=of state] (local) {Current Data Layer\\Typed Mock Data, Local Media and Browser Storage};
  \node[box, below=of local, draw=gray!75, fill=gray!8] (backend) {Planned Connected Backend\\Node.js / Express API and PostgreSQL Database};

  \draw[current] (user) -- node[right, font=\scriptsize] {interacts} (ui);
  \draw[current] (ui) -- node[right, font=\scriptsize] {updates and reads} (state);
  \draw[current] (state) -- node[right, font=\scriptsize] {stores and retrieves} (local);
  \draw[planned] (ui.east) to[out=10,in=10]
    node[right, font=\scriptsize, align=left] {future HTTP/JSON\\requests} (backend.east);
\end{tikzpicture}
\caption{High-level architecture of the current TiTalks prototype and its
planned backend extension. Solid arrows represent the implemented flow; the
dashed arrow represents future integration.}
\label{fig:architecture}
\end{figure}

Figure~\ref{fig:architecture} summarizes the project in a simple flow. The
user interacts with React screens, the screens update client-side state, and
that state reads or writes mock data and browser storage. In the next phase,
the frontend will also communicate with the planned API and database through
the dashed path.
...
\begin{itemize}
  ...
  \item \textbf{Source-file coverage matrix:} An automated inventory checks
    every production TypeScript, TSX, and CSS source file and fails when a file
    is not assigned to at least one unit, integration, build, browser-smoke, or
    staging verification layer. Module-loading cases additionally import every
    application component, data module, and utility to detect module-level
    failures.
...
\begin{tabular}{|p{0.30\textwidth}|p{0.22\textwidth}|p{0.34\textwidth}|}
  ...
  \hline
  Automated test cases & All pass & 54 of 54 passed (100\% pass rate) \\
  \hline
  Test files & All pass & 7 of 7 passed \\
  \hline
  Production source mapping & Every file covered & 39 of 39 files mapped \\
  \hline
...
report, all 54 automated tests passed, all 39 production source files were
mapped to verification layers, and the TypeScript production build
...
\bibitem{react}
Meta Open Source, \emph{React Documentation}, 2026.

\bibitem{typescript}
Microsoft, \emph{TypeScript Documentation}, 2026.

\bibitem{vite}
Vite Team, \emph{Vite Documentation}, 2026.

\bibitem{tailwind}
Tailwind Labs, \emph{Tailwind CSS Documentation}, 2026.

\bibitem{playwright}
Microsoft, \emph{Playwright Documentation}, 2026.
...
\bibitem{openmeteo}
Open-Meteo, \emph{Open-Meteo Weather API Documentation}, 2026.
