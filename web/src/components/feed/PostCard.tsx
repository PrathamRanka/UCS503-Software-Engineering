...
export function PostCard({ post, isLiked, isSaved, onLike, onSave, onShare, currentUsername, archived, onOpen, onOpenProfile, onEdit, onArchive, onDelete, onAction, onUnfollow }: PostCardProps) {
    ...
    <article className="w-full border-b border-neutral-200 bg-white pb-3.5 [content-visibility:auto]">
      <header className="grid h-14 grid-cols-[auto_1fr_auto] items-center gap-2.5 px-3 sm:px-0">
        ...
        <div className="flex min-w-0 flex-col">
          <button className="flex items-center text-left text-sm font-semibold leading-[18px]" onClick={onOpenProfile}>{post.handle}{post.featured ? <span className="ml-1 grid size-[13px] place-items-center rounded-full bg-blue-500 text-[8px] text-white">✓</span> : null}</button>
          <span className="text-xs leading-4 text-neutral-500">{post.place} · {post.time}</span>
        </div>
        <button className={iconButton} onClick={() => setMenuOpen(true)} aria-label="Post menu"><MoreHorizontal size={22} /></button>
      </header>
      ...
      <div className="flex h-12 items-center justify-between px-1 sm:-mx-2 sm:px-0">
        <div className="flex gap-0.5">
          ...
          <button className={iconButton} onClick={onOpen} aria-label="Open comments"><MessageCircle size={25} /></button>
      ...
      <div className="px-3 text-sm leading-[18px] sm:px-0">
        ...
        <button className="block bg-transparent pt-1 text-sm text-neutral-500" onClick={onOpen}>View all {post.comments + comments.length} comments</button>
      ...
      {editing ? <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"><section className="w-full max-w-md rounded-xl bg-white p-5"><h2 className="font-semibold">Edit caption</h2><textarea className="mt-4 min-h-32 w-full rounded-lg border border-neutral-300 p-3 outline-none" value={caption} onChange={(event) => setCaption(event.target.value)} /><div className="mt-4 flex justify-end gap-2"><button className="rounded-lg bg-neutral-100 px-4 py-2 text-sm font-semibold" onClick={() => setEditing(false)}>Cancel</button><button className="rounded-lg bg-blue-500 px-4 py-2 text-sm font-semibold text-white" onClick={() => { onEdit(caption); setEditing(false) }}>Save</button></div></section></div> : null}
      {menuOpen ? <ContentMenuModal own={post.handle === currentUsername} archived={archived} onEdit={() => setEditing(true)} onArchive={onArchive} onDelete={onDelete} onReport={() => onAction('Report submitted')} onBlock={() => onAction(`${post.handle} blocked`)} onMute={() => onAction(`${post.handle} muted`)} onUnfollow={onUnfollow} onClose={() => setMenuOpen(false)} /> : null}
    </article>
  )
}
