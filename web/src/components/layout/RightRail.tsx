...
export function RightRail({ following, onFollow }: RightRailProps) {
  return (
    <aside className="hidden w-80 pt-[50px] xl:block">
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-3">
        <Avatar src={currentUser.avatar} size="lg" />
        <div className="flex min-w-0 flex-col"><strong className="truncate text-sm">{currentUser.handle.slice(1)}</strong><span className="text-sm text-neutral-500">{currentUser.name}</span></div>
        <button className={actionClass}>Switch</button>
      </div>

      <section className="mt-6">
        <div className="flex items-center justify-between"><h2 className="m-0 text-sm font-semibold text-neutral-500">Suggested for you</h2><button className="bg-transparent p-0 text-xs font-semibold">See All</button></div>
