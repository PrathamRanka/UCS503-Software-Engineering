...
function NavButton({ label, icon: Icon, active, badge, dot, avatar, onClick }: NavButtonProps) {
  return (
    <button
    ...
    >
      <span className="relative grid place-items-center transition group-hover:scale-105">
        {avatar ? <Avatar src={currentUser.avatar} size="sm" /> : Icon ? <Icon size={25} strokeWidth={active ? 2.7 : 1.9} /> : null}
...
export function Sidebar({ activeNav, onNavigate, onCreate }: SidebarProps) {
  return (
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-[74px] flex-col items-center border-r border-neutral-200 bg-white px-3 py-8 lg:flex xl:w-[245px] xl:items-stretch">
      ...
      <nav className="mt-9 grid gap-1" aria-label="Main navigation">
        ...
        <NavButton label="Profile" avatar active={activeNav === 'Profile'} onClick={() => onNavigate('Profile')} />
      </nav>
      <div className="mt-auto"><NavButton label="More" icon={Menu} onClick={() => onNavigate('More')} /></div>
    </aside>
  )
}
