      ...
      <div className="relative h-full w-full max-w-[430px] overflow-hidden bg-neutral-950 text-white sm:rounded-lg">
        <div className="absolute inset-x-2.5 top-2 z-10 flex gap-1">
          {Array.from({ length: total }, (_, itemIndex) => (
            ...
            >
              <i
                className={`block h-full bg-white ${itemIndex < index ? "w-full" : itemIndex === index ? `transition-[width] duration-[5000ms] ease-linear motion-reduce:transition-none ${progressed ? "w-full" : "w-0"}` : "w-0"}`}
              />
            </span>
          ))}
        </div>
        <header className="absolute left-4 right-3 top-4 z-10 flex items-center gap-2 drop-shadow-lg">
          <Avatar src={story.image} size="sm" />
          <strong className="text-[13px]">{story.name}</strong>
          <span className="text-[11px] text-white/75">12m</span>
          <button
