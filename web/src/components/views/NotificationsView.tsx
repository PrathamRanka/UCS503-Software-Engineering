      ...
      <div className="px-4 py-7 sm:px-8">
        ...
        <div className="grid gap-1">
          {notifications.map((item, index) => (
            <MotionReveal
              className="flex items-center gap-3 rounded-sm border border-transparent px-3 py-4 transition duration-150 hover:border-black/10 hover:bg-white dark:hover:border-white/10 dark:hover:bg-white/5 motion-reduce:transition-none"
              key={item.id}
            ...
            >
              ...
              {item.follow ? (
                <button
                  className={`rounded-sm px-4 py-2 text-xs font-semibold transition duration-150 active:scale-[.96] motion-reduce:transform-none motion-reduce:transition-none ${following.has(item.id) ? "bg-neutral-100 dark:bg-neutral-800" : "bg-[#ed111c] text-white"}`}
                  onClick={() =>
