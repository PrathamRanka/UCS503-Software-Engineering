  ...
  if (!person)
    return (
      <StatePanel
        ...
        message="This profile does not exist in the demo data."
  ...
  return (
    <section className="mx-auto min-h-screen w-full max-w-4xl">
      <PageHeader
        ...
        action={
          <button onClick={() => setMenu((current) => !current)}>
            <MoreHorizontal />
          </button>
        }
      />
      <div className="px-4 py-7 sm:px-10">
        <div className="grid grid-cols-[90px_1fr] gap-5 sm:grid-cols-[180px_1fr]">
          <div className="flex justify-center">
            <Avatar src={person.avatar} size="lg" />
          </div>
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-xl">{person.username}</h1>
              <button
                className={`rounded-lg px-5 py-2 text-xs font-semibold ${isFollowing ? "bg-neutral-100 dark:bg-neutral-800" : "bg-blue-500 text-white"}`}
                onClick={() => onFollow(handle)}
              >
                {isFollowing ? "Following" : "Follow"}
              </button>
              <button
                className="rounded-lg bg-neutral-100 dark:bg-neutral-800 px-5 py-2 text-xs font-semibold"
            ...
            <div className="mt-5 flex gap-8 text-sm">
              ...
              <button onClick={() => setList("Followers")}>
                <b>{person.followers.toLocaleString()}</b> followers
              </button>
        ...
        <div className="mt-10 flex h-12 items-center justify-center gap-2 border-t border-neutral-200 dark:border-neutral-800 text-xs font-semibold">
          <Grid3X3 size={15} />
          POSTS
        </div>
        <div className="grid grid-cols-3 gap-0.5 sm:gap-1">
          {exploreItems.slice(0, 9).map((item) => (
            <button
        ...
        >
          ...
          >
            {[
              ...
              <button
                className={`w-full border-b border-neutral-100 dark:border-neutral-800 p-4 text-sm font-semibold ${action === "Report" || action === "Block" ? "text-red-500" : ""}`}
                onClick={() => {
                  action === "Follow" || action === "Unfollow"
                    ? onFollow(handle)
                    : onAction(`${person.username} ${action.toLowerCase()}ed`);
                  setMenu(false);
                }}
                key={action}
              >
