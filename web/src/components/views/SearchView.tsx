...
export function SearchView({ following, onFollow }: SearchViewProps) {
  ...
  const results = useMemo(() => {
    ...
    return normalized ? people.filter((person) => `${person.name} ${person.username} ${person.branch}`.toLowerCase().includes(normalized)) : people.slice(1)
  }, [query])
  ...
  return (
    <section className="mx-auto min-h-screen w-full max-w-3xl border-x border-neutral-100">
      ...
      <div className="p-4 sm:p-6">
        <label className="flex h-11 items-center gap-3 rounded-lg bg-neutral-100 px-4"><Search size={19} className="text-neutral-500" /><input className="min-w-0 flex-1 bg-transparent text-sm outline-none" value={query} onChange={(event) => setQuery(event.target.value)} autoFocus placeholder="Search students, clubs and branches" />{query ? <button onClick={() => setQuery('')} aria-label="Clear search"><X size={17} /></button> : null}</label>
        <h2 className="mb-3 mt-7 text-sm font-semibold">{query ? 'Results' : 'Suggested'}</h2>
